...
class SnakeGame:
    def __init__(self):
        ...
        pygame.display.set_caption('Snake Practice 11 - Wrap Around')
    ...
    def display_score(self):
        value = self.font_style.render(f"Score: {self.score}  Level: {self.level}  Weight: {self.food_weight}", True, WHITE)
    ...
    def run(self):
        while not self.game_over:
            ...
            if current_time - self.food_timer > self.food_lifetime:
                self.generate_food()
            ...
            # --- ҚАБЫРҒАДАН ӨТУ ЛОГИКАСЫ ---
            if self.snake_pos[0] >= WIDTH: self.snake_pos[0] = 0
            elif self.snake_pos[0] < 0: self.snake_pos[0] = WIDTH - BLOCK_SIZE
            
            if self.snake_pos[1] >= HEIGHT: self.snake_pos[1] = 0
            elif self.snake_pos[1] < 0: self.snake_pos[1] = HEIGHT - BLOCK_SIZE
            # ------------------------------
